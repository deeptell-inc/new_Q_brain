...
systems---shows that a register whose dipolar vector reorients only with the whole
protein cannot reach the neural band
at all: at the geomagnetic field the system is in extreme narrowing, a proton on a
$60$~kDa protein relaxes in $2.4$~ms, and the horizon is capped at $6.1$~ms. The
register must reorient about two and a half times faster than the whole protein for the
...
$\tau_c\lesssim6$~ns, at a turnover interval set by that mobility ($9.8$--$27$~ms
at $\tau_c=5$~ns, up to $5.4$--$1340$~ms at $0.1$~ns)---and both of its parameters are
...
covered by an automated test suite (290 tests, all passing): twenty-nine bind the
headline quantities and one hundred and forty bind the capacity and
